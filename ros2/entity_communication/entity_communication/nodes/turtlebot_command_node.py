...
class TurtleBotCommandNode(Node):
    def __init__(self,connection_type,robot):
        super().__init__('turtlebot_command_node')
        self.name = 'turtlebot_command_node'
        self.robot = robot
        self.status = 0
        ...
        if robot == 'turtlebot':
            self.declare_parameter('turtlebot/ip')
            ip = str(self.get_parameter('turtlebot/ip').value)
        else:
            ip=None
    ...
    def tear_down(self):
        try:
            self.destroy_node()
            rclpy.shutdown()
            print(cl_green("Successfully tore down turtlebot node"))
        except:
...
def main(argv=sys.argv[1:]):
    ...
    while True:
        ...
        turtlebot_command_node = TurtleBotCommandNode(args.connection,args.robot)

        rclpy.spin(turtlebot_command_node)
